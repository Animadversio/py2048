...
#%%
plt.figure()
data4 = np.load(r"E:\Github_Projects\2048\exp_data\DQN2_3dCNN_26M_scores.npz",)
scorevec = data4["eps_rew"]
plt.hist(scorevec, bins=65, label="DQN buffer+batch+", alpha=0.4)
plt.axvline(x=scorevec.mean(), label=f"DQN buffer+batch+: {scorevec.mean():.0f}+-{scorevec.std():.0f}", ls="-", color="magenta")
