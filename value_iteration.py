...
from approximator import policy_CNN, Value_CNN, Adam
#%
def update_value_iter(Vnet, Voptim, target_buffer,
               update_step_freq=40000, K_epochs=100,
               writer=None, global_step=0, gamma=0.99,):
    Vnet.train()
    actseq = []
    rewardseq = []
    stateseq = []
    ...
    for runi in range(len(target_buffer)):
        ...
        if len(actseq) > update_step_freq \
            ...
            reward2go_vec = torch.tensor(reward2go_vec).float().cuda()
...
def evaluate_value_iter(Vnet, target_buffer,
    ...
    for runi in range(len(target_buffer)):
        ...
        if len(actseq) > update_step_freq \
            ...
            rew2go_all.append(reward2go_vec.detach().cpu())
    ...
    return valL2_mean, valL1_mean, value_corr
#%%
expmax_buffer = {}
...
#%%
Vnet = Value_CNN().cuda()
Voptim = Adam([*Vnet.parameters()], lr=0.0005)
#%%
writer = SummaryWriter("logs\\value_iter_pilot")
global_step = 0
#%%
update_step_freq = 75000
for cycle in range(33, 80):
    # update model
    global_step = update_value_iter(Vnet, Voptim, expmax_buffer,
                         update_step_freq=update_step_freq, K_epochs=50,
                         writer=writer, global_step=global_step, gamma=0.99)
    if cycle % 2 == 0:
        torch.save(Vnet.state_dict(), f"ckpt\\value_iter\\Vnet_iter{cycle:d}_gs{global_step:d}.pt")

    valL2_mean, valL1_mean, value_corr = evaluate_value_iter(Vnet, expmax_buffer,
               update_step_freq=update_step_freq, writer=writer,
               global_step=global_step, gamma=0.99,)
